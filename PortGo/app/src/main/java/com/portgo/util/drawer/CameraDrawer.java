package com.portgo.util.drawer;
...
import com.portgo.R;
import com.portgo.util.filter.AFilter;
import com.portgo.util.filter.CameraFilter;
import com.portgo.util.filter.GroupFilter;
import com.portgo.util.filter.NoFilter;
import com.portgo.util.filter.ProcessFilter;
import com.portgo.util.filter.WaterMarkFilter;
import com.portgo.util.gpufilter.SlideGpuFilterGroup;
import com.portgo.util.gpufilter.filter.MagicBeautyFilter;
import com.portgo.util.record.video.TextureMovieEncoder;
import com.portgo.util.EasyGlUtils;
...
//    private MagicAntiqueFilter mBeautyFilter;
    ...
    /**预览数据的宽高*/
    private int mPreviewWidth=0,mPreviewHeight=0,mOrientation=0;
    /**控件的宽高*/
    ...
    private boolean SPWSR = false;//stopPreviewWhenStopRecord
    ...
    public CameraDrawer(@NonNull Context context){
        ...
        drawFilter = new CameraFilter(context);

        mProcessFilter=new ProcessFilter(context);
        mBeFilter = new GroupFilter(context);
        mAfFilter = new GroupFilter(context);
        mBeautyFilter = new MagicBeautyFilter(context);
//        mBeautyFilter = new MagicAntiqueFilter();
        mSlideFilterGroup = new SlideGpuFilterGroup(context);
        ...
        //水印滤镜，去除
        WaterMarkFilter waterMarkFilter = new WaterMarkFilter(context);
        waterMarkFilter.setWaterMark(BitmapFactory.decodeResource(context.getResources(),R.drawable.logo));
        waterMarkFilter.setPosition(3,5,20,5);
...
//        showFilter.setTextureId(textureID);
    ...
    public void setOutputOrientation(int orientation){
        mOrientation =orientation>0?orientation:0;
    }
            ...
            switch (recordingStatus){
                case RECORDING_OFF:
                    ...
                    videoEncoder.startRecording(new TextureMovieEncoder.EncoderConfig(
                            ...
                            3500000,mOrientation, EGL14.eglGetCurrentContext(),
                            null));
            ...
            switch (recordingStatus) {
                ...
                case RECORDING_PAUSED:
                    if(onStopRecordingListener!=null){
                        onStopRecordingListener.onStopRecording(recordingStatus);
                    }
...
//        result.copyPixelsFromBuffer(IntBuffer.wrap(ib.array()));
    ...
    public void setOnStopRecordingListener(OnStopRecordingListener listener){
        this.onStopRecordingListener = listener;
    }
    ...
    public void setCameraId(int id) {
        ...
        if(id==0){//后置摄像头，上下颠倒，左右景象
            switch (info.orientation){
                case 270:
                    MatrixUtils.resetMatrix(OM);
                    MatrixUtils.flip(OM, true, true);//矩阵上下翻转
                    break;
                case 90:
                    MatrixUtils.resetMatrix(OM);
                    break;
    ...
    public interface OnStopRecordingListener{
        void onStopRecording(int recordingStatus);
    }
}
