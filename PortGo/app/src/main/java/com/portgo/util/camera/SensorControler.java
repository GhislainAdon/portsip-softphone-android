...
public class SensorControler implements SensorEventListener {
    ...
    private SensorListener mCameraFocusListener;
    ...
    public void setCameraFocusListener(SensorListener mCameraFocusListener) {
    ...
    private void getOriatation(){
        float[] values= new float[3];
        if(gravity!=null && geomagnetic!=null&&mCameraFocusListener!=null) {
            if(SensorManager.getRotationMatrix(r, null, gravity, geomagnetic))
            {
                SensorManager.getOrientation(r, values);
                values[0] = (float) ((360f+values[0]*180f/Math.PI)%360);
                values[1] = (float) ((360f+values[1]*180f/Math.PI)%360);
                values[2] = (float) ((360f+values[2]*180f/Math.PI)%360);

                mCameraFocusListener.oriatationChanger(values);
            }
        }
    }
    @Override
...
//                PortApplication.getLogUtils().d(TAG, "pX:" + px + "  pY:" + py + "  pZ:" + pz + "    stamp:"
//                        + stamp + "  second:" + second);
                double value = Math.sqrt(px * px + py * py + pz * pz);
                if (value > 1.4) {
//                    textviewF.setText("检测手机在移动..");
//                    PortApplication.getLogUtils().i(TAG,"mobile moving");
                    STATUE = STATUS_MOVE;
                } else {
//                    textviewF.setText("检测手机静止..");
//                    PortApplication.getLogUtils().i(TAG,"mobile static");
                    //上一次状态是move，记录静态时间点
...
//                                onCameraFocus();
                                if (mCameraFocusListener != null) {
                                    mCameraFocusListener.stayAfterMove();
                                }
//                                PortApplication.getLogUtils().i(TAG,"mobile focusing");
                            }
                        }
                    }
    ...
    public interface SensorListener {
        void stayAfterMove();
        void oriatationChanger(float[]values);
    }
}
