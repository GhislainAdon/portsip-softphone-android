package com.portgo.util.gpufilter;
...
import com.portgo.util.gpufilter.helper.MagicFilterFactory;
...
public class SlideGpuFilterGroup {
    ...
    public SlideGpuFilterGroup(@NonNull Context context) {
        mContext =context;
        initFilter();
    ...
    private GPUImageFilter getFilter(int index) {
        GPUImageFilter filter = MagicFilterFactory.initFilters(mContext,types[index]);
        if (filter == null) {
            filter = new GPUImageFilter(mContext);
        }
        return filter;
    }
